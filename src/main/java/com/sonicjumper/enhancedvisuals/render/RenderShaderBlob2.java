...
public class RenderShaderBlob2 extends RenderShaderDefault {
			...
			if(group != null)
			{
				for(Shader mcShader : group.getShaders()) {
					...
					if (shaderuniform != null) {
						Float currentBlurRadius = (float) Math.floor((v.getTranslucencyByTime() * (sbf.getRadius() - 1.0F)) + 1.0F);
